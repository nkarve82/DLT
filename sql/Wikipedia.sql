...
CREATE LIVE TABLE clickstream_bad(
  CONSTRAINT valid_current_page EXPECT (current_page_id IS NULL OR current_page_title IS NULL) ON VIOLATION DROP ROW
  --,  CONSTRAINT valid_count EXPECT (click_count <= 0) ON VIOLATION DROP ROW
)
COMMENT "Wikipedia clickstream dataset failing quality expectations."
TBLPROPERTIES ("quality" = "failed")
AS SELECT
  CAST (curr_id AS INT) AS current_page_id,
  curr_title AS current_page_title,
  CAST(n AS INT) AS click_count,
