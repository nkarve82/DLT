...
CREATE INCREMENTAL LIVE TABLE customers4
...
--FROM cloud_files("abfss://devc-dlt-demo@dvcpmdpadbadls.dfs.core.windows.net/retail-org/customers/", "csv")
FROM cloud_files("/mnt/devc-dlt-demo2/retail-org/customers/", "csv")
...
SELECT * , current_timestamp() as UpdatedDate2 FROM cloud_files("/databricks-datasets/retail-org/sales_orders/", "json", map("cloudFiles.inferColumnTypes", "true"))

-- COMMAND ----------

CREATE INCREMENTAL LIVE TABLE sales_orders_streamed(
...
AS
SELECT *  , current_timestamp() as UpdatedDate3
  FROM STREAM(LIVE.sales_orders_raw)
